...
| 2b | Apuntar la RPi5 al broker de la VM (`.env`: `MQTT_HOST`/`MQTT_PORT`/credenciales) | Aldana | 🔴 **Bloqueado** — la RPi5 no es alcanzable desde la sesión que hizo 1/2 (fuera de la red del LIT, sin Tailscale local). Instrucciones exactas ya en `docs/PROGRESS.md` de `starlink-measurement-station`, sección "Broker MQTT compartido desplegado..." | Que la RPi5 empiece a recibir datos de Fede también |
| 2c | Apuntar el mock/firmware de Fede al broker de la VM | Fede | 🔴 Sin empezar — necesita las credenciales (pedirlas a Aldana, no están en ningún repo) | Primer mensaje real de Fede llegando a la VM |
